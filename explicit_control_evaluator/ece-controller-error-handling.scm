...
(define ece-controller
  '(*read-eval-print-loop
    ...
    *ev-cond-loop
    (test (op cond-one-clause?) (reg unev))
    (branch (label *ev-cond-one-clause))
    (save unev)
    (assign unev (op cond-first-clause) (reg unev))
    (test (op cond-else-clause?) (reg unev))
    (goto (label *ill-formed-special-form))
    ...
    *ill-formed-special-form
    (assign val (const ill-formed-special-form-type-error))
    (goto (label *signal-error))
    *unbound-variable
    (assign val (const unbound-variable-error))
    (goto (label *signal-error))
    *unsafe-application
    (assign val (const unsafe-application-error))
    (goto (label *signal-error))
    *signal-error
    (perform (op user-print) (reg val))
    (goto (label *read-eval-print-loop))    
    ...
    *primitive-apply
    ...
    (test (op unsafe-to-apply?) (reg val))
